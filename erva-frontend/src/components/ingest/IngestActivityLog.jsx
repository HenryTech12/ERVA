...
const CHANNEL_COLORS = {
  transfer: 'bg-[#1B2540] text-[#8891A8] border-[#26314D]',
...
function ChannelBadge({ channel }) {
  ...
  const cls = CHANNEL_COLORS[ch] ?? 'bg-[#1B2540] text-[#8891A8] border-[#26314D]'
  return (
...
export function IngestActivityLog({ transactions, isLoading, onRowClick }) {
  ...
  const SortIcon = ({ field }) => {
    if (sortField !== field) return <span className="text-[#26314D]">↕</span>
    return <span className="text-[#4FA0A0]">{sortDir === 'asc' ? '↑' : '↓'}</span>
  }
  ...
  return (
    <div className="bg-[#131B2E] border border-[#26314D] rounded-lg overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-[#26314D] flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 flex-1 min-w-48 rounded-md focus-within:ring-2 focus-within:ring-[#4FA0A0]/50 px-1 -mx-1">
          ...
          <input
            ...
            className="bg-transparent text-sm text-[#E8EAF0] placeholder:text-[#8891A8] focus:outline-none w-full font-mono"
          />
        </div>
        <div className="flex items-center gap-2">
          <FunnelIcon className="w-3.5 h-3.5 text-[#8891A8]" />
          <select
            value={channelFilter}
            onChange={(e) => setChannelFilter(e.target.value)}
            className="bg-[#1B2540] border border-[#26314D] rounded px-2 py-1 text-xs text-[#E8EAF0] focus:outline-none focus:border-[#4FA0A0]/50"
          >
        ...
        <span className="text-xs text-[#8891A8] ml-auto">{filtered.length} records</span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-[#8891A8] text-sm">No transactions match your filters</div>
      ) : (
              ...
              <tr>
                {[
                  ...
                  <th
                    ...
                    className={cn(
                      'px-4 py-3 text-left text-[10px] uppercase tracking-wider text-[#8891A8] font-medium',
                      field && 'cursor-pointer hover:text-[#E8EAF0] select-none'
                    )}
                  >
                  ...
                  <tr
                    ...
                    className={cn(
                      ...
                      isHighRisk
                        ...
                        : 'bg-[#131B2E] hover:bg-[#1B2540]'
                    )}
                  >
                    <td className="px-4 py-3 text-[#8891A8] font-mono text-xs whitespace-nowrap">
                      {formatDateTime(txn.date)}
                    </td>
                    <td className="px-4 py-3 text-xs max-w-[140px]">
                      <p className="text-[#E8EAF0] truncate font-medium">
                        {truncate(txn.fromEntityName ?? txn.fromEntity)}
                      </p>
                      <p className="text-[#8891A8] font-mono text-[10px] truncate">{truncate(txn.fromEntity, 14)}</p>
                    </td>
                    <td className="px-4 py-3 text-xs max-w-[140px]">
                      <p className="text-[#E8EAF0] truncate font-medium">
                        {truncate(txn.toEntityName ?? txn.toEntity)}
                      </p>
                      <p className="text-[#8891A8] font-mono text-[10px] truncate">{truncate(txn.toEntity, 14)}</p>
                    </td>
                    <td className="px-4 py-3 text-[#4FA0A0] font-mono text-xs whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 text-[#8891A8] font-mono text-[10px] max-w-[120px] truncate">
                      {txn.reference ?? '—'}
                    </td>
                  </tr>
                )
