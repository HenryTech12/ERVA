...
def _execute_enforcement(db: Session, draft: STRDraft) -> EnforcementResult:
    ...
    # Get or create the ERVA quarantine account
    quarantine = db.scalar(
        select(Entity).where(Entity.metadata_json["external_id"].astext == "ERVA-QUARANTINE")
    )
    if quarantine is None:
        quarantine = Entity(
            entity_type="institution",
            full_name="ERVA Compliance Quarantine Account",
            metadata_json={"external_id": "ERVA-QUARANTINE", "system": True},
        )
        db.add(quarantine)
        db.flush()
